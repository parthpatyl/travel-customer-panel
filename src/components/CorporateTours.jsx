  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-amber-900 text-white">
        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1540575467063-178a50c2df87?q=80&w=2070')] bg-cover bg-center opacity-20" />
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4">
          <h2 className="text-2xl sm:text-3xl font-bold text-center text-slate-900 mb-12">
            Why Kraft Your Trip for Corporate?
          </h2>
                    ...
                    {corporateTestimonials.map((_, i) => (
                      <button
                        ...
                        className={`w-2.5 h-2.5 rounded-full transition-all ${
                          i === clampedTestiIndex ? 'bg-amber-500 w-6' : 'bg-gray-300 hover:bg-gray-400'
                        }`}
                      />
                    ))}
                  </div>
              ...
              {['all', 'india', 'international'].map((f) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                    filter === f ? 'bg-amber-500 text-white' : 'bg-white text-slate-600 hover:bg-gray-100'
                  }`}
                >
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredPackages.map((pkg) => (
                <div key={pkg.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
                  ...
                  <div className="p-6">
                    <span className={`inline-block text-xs font-semibold uppercase tracking-wider px-2.5 py-1 rounded-full mb-3 ${
                      pkg.category === 'india' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'
                    }`}>
